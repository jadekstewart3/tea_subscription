...
describe "Customers API" do 
  describe "Customers Show" do 
    describe "when successful" do
      it "can get a customer show page with all active and inactive subscriptions" do 
    ...
    describe "when unsuccessful" do 
      it "returns an error if the customer does not exist" do 
        get "/api/v1/customers/18390248324"

        customer_response = JSON.parse(response.body, symbolize_names: true)

        expect(response).to_not be_successful
        
        expect(customer_response).to have_key(:message)
        expect(customer_response[:message]).to eq("your query could not be completed")

        expect(customer_response).to have_key(:errors)
        expect(customer_response[:errors]).to be_an(Array)

        customer_response[:errors].each do |error|
          expect(error).to have_key(:status)
          expect(error[:status]).to eq("404")

          expect(error).to have_key(:title)
          expect(error[:title]).to eq("Couldn't find Customer with 'id'=18390248324")
        end
      end
    end
  end
